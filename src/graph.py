...
class Workflow():
    def __init__(self):
        ...
        workflow.add_node("save_draft_email", nodes.save_draft_email)
        ...
        workflow.add_conditional_edges(
            ...
            nodes.route_email_based_on_category,
            {
                ...
                "not product related": "assemble_context",  # Feedback or Complaint - still needs context
        ...
        # assemble context after RAG retrieval
        workflow.add_edge("retrieve_from_rag", "assemble_context")
        # give information to writer agent to create draft email
        workflow.add_edge("assemble_context", "email_writer")
        ...
        workflow.add_conditional_edges(
            ...
            nodes.must_rewrite,
            {
                "send": "save_draft_email",
                "rewrite": "assemble_context",  # Reassemble context before rewrite
                "stop": "save_draft_email"  # Save draft even if max trials reached
            }
        )
        
        # Save draft email and then check for more emails
        workflow.add_edge("save_draft_email", "is_email_inbox_empty")
